...
import org.firstinspires.ftc.teamcode.utility.math.MatBuilder;
...
public class Rotation3d implements Interpolatable<Rotation3d> {
    ...
    public Rotation3d(Vector<N3> initial, Vector<N3> last) {
        ...
        if (dotNorm > 1.0 - 1E-9) {
            ...
            final QRDecomposition<DMatrixRMaj> qr = DecompositionFactory_DDRM.qr(3, 1);
            qr.decompose(X.getStorage().getMatrix());
            final DMatrixRMaj Q = qr.getQ(null, false);
